...
#
# RELEASE_BUNDLE_ID overrides the bundle id VALUE; IDENTITY_XCCONFIG overrides
# the FILE it is read from, for red controls only, and announces itself.
#
...
#
# IDENTITY_XCCONFIG overrides the FILE, where RELEASE_BUNDLE_ID overrides the
# VALUE. It exists so this gate's failure paths can be driven red against a
# fixture without editing the tracked config, and it ANNOUNCES ITSELF when set
# — the same discipline as bin/preflight-identity.rb's --config banner, for the
# same reason: a run pointed at a fixture must never be mistakable for a
# release run in a log. CI never sets it. A gate whose red path cannot be
# exercised is a gate nobody has seen fail, and this repository has shipped
# more than one check whose pass condition was met regardless of what it
# claimed to detect.
#
