...
| Do **not** sed-substitute `HelloApp` literals | The template resolves identity everywhere it matters | Edit the four keys of `app/Identity.xcconfig` — `APP_PRODUCT_NAME`, `BUNDLE_ID`, `DISPLAY_NAME`, `COPYRIGHT`. Both generators read them as `$(VAR)`. `bin/rename.sh --email=…` personalizes the contact address and repo slug and nothing else; a fork created before that file existed migrates with `ruby bin/migrate-identity.rb` (see `docs/MIGRATING-FROM-RENAME.md`). `.bootstrap.env` keeps its own `APP_NAME` / `BUNDLE_ID` handle, which the Fastfile and the release workflows read; it is not where the build gets its identity |
...
| Real-device UI automation (opt-in) | `bin/device-rig/` | Appium + WebDriverAgent, for what XCUITest cannot reach from inside your target: the share sheet, Settings, Messages, any number of phones at once. Template-owned; **not** wired into `make check`/`make verify`. Read `docs/UI-AUTOMATION.md` before using it — a dev-signed WDA expires every 7 days and presents as a capability failure |
...
- Your app code lives under `app/`, which the template never touches (`app/Shared/App.swift` is the `@main` entry point and keeps its name on every fork)
