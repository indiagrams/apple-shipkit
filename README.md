...
├── bin/device-rig/              # opt-in real-device automation (see docs/UI-AUTOMATION.md)
...
- **[docs/UI-AUTOMATION.md](docs/UI-AUTOMATION.md)** — two layers of UI testing. In-process XCUITest (already wired) plus the two helpers that remove its sharp edges: run against a connected iPhone instead of a simulator, and get the screenshot out of a failed `.xcresult` — `--only-failures` silently returns nothing for a `tearDown` screenshot. Then an **opt-in** device rig (Appium + WebDriverAgent) for what XCUITest cannot reach from inside your own target: the share sheet, Settings, a universal link tapped for real in Messages, any number of phones at once. Carries the OS-version-specific landmines, each measured rather than assumed.
